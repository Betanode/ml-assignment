...
function getVideo2(){
    video.play();
    const chunks = [];
    const rec = document.getElementById("vv");
    rec.start();
    rec.ondataavailable = e => chunks.push(e.data)
    if (check) {
        poseNet.on("pose", function(results) {
...
function checkReps(pose) {
  ...
  if (leftSHKAngle >= 0.9 && rightSHKAngle >= 0.9 && 
  ...
  if (Math.abs(leftHKVec.y) < 100 && Math.abs(rightHKVec.y) < 100) {
